# =============================================================================
# within_2018_final.py
# =============================================================================
# - Fixes ROC-AUC NaN for binary classification
# - Fixes SHAP for binary models
# - Uses only numeric columns
# - Robust SMOTE and probability alignment
# =============================================================================
...
# =============================================================================
# SHAP NORMALISER (handles binary correctly)
# =============================================================================
def _get_shap_list(raw, n_classes, model=None):
    """
    Normalise SHAP output into a list of arrays (one per class).
    For binary classification, we get a single 2-D array.
    We convert to two arrays: negative (class 0) and positive (class 1).
    The sign is determined by model.classes_ order.
    """
    # If raw is a list, first flatten
    if isinstance(raw, list):
        if len(raw) == 1:
            arr = raw[0]
        else:
            # Already a list of arrays per class (multiclass)
            return [np.array(item) for item in raw]

    arr = np.array(raw)

    if arr.ndim == 3:
        # Multiclass, already in shape (n_samples, n_features, n_classes) or similar
        ...
        if model is not None and hasattr(model, 'classes_'):
            # model.classes_ is sorted: [0,1] for binary
            if len(model.classes_) == 2 and model.classes_[0] == 0 and model.classes_[1] == 1:
                # typical: class0 => negative, class1 => positive
                # SHAP values for class1 are usually positive for that class,
                # and for class0 they are the negative of class1 (in some models).
                # To be safe, we use the model's internal order:
                # For XGBoost, shap_values for binary returns a single array for the positive class.
                # We'll treat that as positive (class1), and negative is -positive.
                return [-arr, arr]
            else:
                # Fallback: assume first class is negative, second positive
                return [-arr, arr]
        else:
            # Fallback: assume class1 is positive
            return [-arr, arr]
...
# =============================================================================
# PER-SAMPLE SHAP EXPLANATIONS (now uses _get_shap_list)
# =============================================================================
...
# =============================================================================
# FIXED METRICS – handles binary ROC-AUC correctly
# =============================================================================
def compute_metrics_fixed(y_true, y_pred, y_prob, le, model=None):
    ...
    # Align probabilities to full label encoder order
    if model is not None and hasattr(model, 'classes_'):
    ...
    # ROC-AUC: if binary (n_cls == 2), use probabilities of the positive class (index 1)
    if len(set(y_true)) < 2:
        roc_auc = float('nan')
    else:
        try:
            if n_cls == 2:
                # Binary case: y_prob should have two columns, use the second
...
# =============================================================================
# MAIN EVALUATION (within 2018)
# =============================================================================
def run_within_2018():
    print("\n" + "="*70)
    print("  WITHIN-DATASET EVALUATION – CIC-IDS-2018 (FIXED ROC-AUC & SHAP)")
    print("  Train 80%, Test 20%")
    print("="*70)

    if not os.path.exists(CIC2018_CSV):
        print(f"❌ File not found: {CIC2018_CSV}")
        return None

    df = load_dataset(CIC2018_CSV)
    ...
    # SMOTE with cap – target = max(original, target_size)
    if SMOTE_AVAILABLE:
        from collections import Counter
        counts = Counter(y_train)
        max_count = max(counts.values())
        target_size = min(MAX_CLASS_SAMPLES, max_count)
        target_size = max(target_size, 100000)
    ...
    # Summary
    print("\n" + "="*70)
    print("  SUMMARY OF ALL MODELS (CIC-IDS-2018 WITHIN)")
    print("="*70)
    summary_df = pd.DataFrame(all_results).T.round(4)
    print(summary_df)
    summary_df.to_csv(os.path.join(OUTPUT_DIR, "within_2018_results.csv"))
    ...
    plt.title("Confusion Matrix – Ensemble (CIC-2018 Within)")
    plt.xlabel("Predicted")
    plt.ylabel("True")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "cm_2018_within.png"), dpi=150)
    plt.close()

    # Global SHAP (using XGBoost) – now fixed
    ...
    try:
        ...
        plt.title("SHAP Feature Importance (XGBoost – CIC-2018 Within)")
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, "shap_global_2018_within.png"), dpi=150)
        plt.close()
    ...
    }).head(1000).to_csv(os.path.join(OUTPUT_DIR, "predictions_2018_within.csv"), index=False)

    print(f"\nAll outputs saved to: {OUTPUT_DIR}")
    return all_results

if __name__ == "__main__":
    results = run_within_2018()
    if results is not None:
        print("\nFinal within-2018 results:")
